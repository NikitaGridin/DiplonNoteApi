...
class adminController {
   async checkSubscribe(req, res, next){
    ...
    const isSubscribedToUser = await connectionsService.checkSubscribe(subscriberId, userId);
    // проверяем, подписан ли другой пользователь на пользователя с ID userId
    const isOtherUserSubscribed = await connectionsService.checkSubscribe(userId, subscriberId);
   ...
   async changeSubscribe(req, res, next){
    ...
    try {
      const subscribe = await connectionsService.changeSubscribe(id,userId,status);
