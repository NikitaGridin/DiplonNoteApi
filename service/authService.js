...
class authService {
  ...
  async signIn(body, img, next) {
    ...
    if (userEmail) {
      throw Object.assign(new Error(`Данный Email занят!`), { statusCode: 409 });
    }
    const userNickname = await User.findOne({ where: { nickname } });
    if (userNickname) {
      throw Object.assign(new Error(`Данный Nickanme занят!`), { statusCode: 409 });
    }
    if(password != password_repeat){
      throw Object.assign(new Error(`Пароли не совпадают!`), { statusCode: 409 });
    }
  ...
  async logIn(body){
    try {
      ...
      if(user.isActivation === false){
        throw Object.assign(new Error("Аккаунт не активирован!"), { statusCode: 400 });
      }
  ...
  async activate(body){
    try{
      ...
      const user = await User.findOne({where: {activationCode: activationCode}});
  ...
  async refresh(refreshToken){
    try{
      ...
      if(!refreshToken){
        throw Object.assign(new Error(`Пользователь не авторизован`), { statusCode: 404 });
      }
